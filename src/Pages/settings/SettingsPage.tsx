...
import { LogOut, User, CreditCard, Store, Trash2, ChevronRight, Crown } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-app pb-24 font-sans antialiased">
      ...
      <header className="bg-ink-800 text-white px-4 pt-8 pb-6 sticky top-0 z-10 shadow-md">
        <h1 className="text-lg font-black tracking-tight text-foam-100">Mon Compte</h1>
        <p className="text-teal-300/60 text-xs mt-0.5 font-medium">Paramètres & abonnement</p>
      </header>
      ...
      <div className="px-4 mt-5 flex flex-col gap-4">
        ...
        <div className="bg-card rounded-2xl border border-slate-200 shadow-card overflow-hidden">
          <div className="px-4 pt-4 pb-4 flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-teal-700/10 border border-teal-200/60 flex items-center justify-center shrink-0">
              {displayName ? (
                <span className="text-sm font-black text-teal-700">{getInitials(displayName)}</span>
              ) : (
        ...
        {shop && (
          <div className="bg-card rounded-2xl border border-slate-200 shadow-card overflow-hidden">
            <div className="px-4 pt-3 pb-1">
              <p className="text-micro font-bold text-slate-400 uppercase tracking-wider">Boutique active</p>
            </div>
            <div className="px-4 pb-4 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-400 capitalize mt-0.5">{shop.work}</p>
              </div>
              <span className={`ml-auto text-micro font-bold px-2.5 py-1 rounded-full shrink-0 ${planStyle.color}`}>
                {planStyle.label}
              </span>
            </div>
          </div>
        ...
        {isGerant && (
          <div className="bg-card rounded-2xl border border-slate-200 shadow-card overflow-hidden">
            <div className="px-4 pt-3 pb-1">
              <p className="text-micro font-bold text-slate-400 uppercase tracking-wider">Abonnement</p>
            </div>
            <button
            ...
            >
              ...
              <div className="text-left min-w-0">
                <p className="font-bold text-slate-800 text-sm">
                  {isGratuit ? "Passer à la version payante" : "Voir les offres"}
                </p>
                <p className="text-xs text-slate-400 mt-0.5">
                  {isGratuit ? "Boutique 19€/mois · Multi 39€/mois" : `Plan ${planStyle.label} actif`}
                </p>
              </div>
            ...
            {!isGratuit && (
              ...
              >
                ...
                <div className="text-left min-w-0">
                  <p className="font-bold text-slate-700 text-sm">Facturation & résiliation</p>
                  <p className="text-xs text-slate-400 mt-0.5">Portail Stripe sécurisé</p>
                </div>
                <ChevronRight size={15} className="text-slate-300 ml-auto shrink-0" />
              </button>
            )}
        ...
        >
          ...
          <span className="font-bold text-slate-800 text-sm">Se déconnecter</span>
          <ChevronRight size={15} className="text-slate-300 ml-auto shrink-0" />
        </button>
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-500 leading-relaxed mt-2">
                Cette action est <strong>irréversible</strong>. Toutes vos boutiques, lots et
                données seront supprimés. Si vous avez un abonnement actif, il sera
